...
describe('runConfigure exit codes', () => {
  ...
  it('exits 1 and reports problems when a HARD runbook step fails', async () => {
    ...
    const result = await runConfigure({ cwd: repo.root, homeDir: repo.root, env: {}, now: NOW, run: oldGit });
  ...
  it('surfaces a not-a-repo context error as exit 1 instead of exit 0', async () => {
    ...
    const deps: CliDeps = { cwd: '/anywhere', homeDir: '/anywhere', env: {}, now: NOW, run: gitNotARepo, runRaw: gitNotARepo };
